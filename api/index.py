...
async def get_gemini_response(prompt: str, image: Optional[Image.Image] = None, expect_json: bool = False):
    ...
    for model_name in models_to_try:
        for attempt in range(1): 
            try:
                # Force JSON mode for better reliability if expected
    ...
    raise Exception(f"API/Quota error: {last_error[:30]}")
...
async def analyze_product(file: UploadFile = File(...)):
    ...
    try:
        response_text = await get_gemini_response(prompt, image, expect_json=True)
...
async def get_news():
    ...
    """
    try:
        response_text = await get_gemini_response(prompt, expect_json=True)
        data = extract_json(response_text)
...
# Keep this so we can run `python api/index.py` locally if we don't use `vercel dev`
